...
const swsOpenObserveEmitter = require("./swsOpenObserveEmitter");
...
class SwsProcessor {
    constructor() {
        ...
        // OpenObserve Emitter
        this.openobserveEmitter = new swsOpenObserveEmitter();
    }
    ...
    init() {
        ...
        this.openobserveEmitter.initialize(swsSettings);
    ...
    tick(that) {
        ...
        that.openobserveEmitter.tick(ts, totalElapsedSec);
    }
    ...
    collectRequestResponseData(res) {
        ...
        if ("sws" in req) {
            ...
            if ("deprecated" in req.sws)
                rrr.api.deprecated = req.sws.deprecated;
            if ("operationId" in req.sws)
                rrr.api.operationId = req.sws.operationId;
            if ("tags" in req.sws) rrr.api.tags = req.sws.tags;

            // Get API parameter values per definition in swagger spec
            var apiParams = this.apiStats.getApiOpParameterValues(
                req.sws.api_path,
                req.method,
                req,
                res
    ...
    processResponse(res) {
        ...
        if (!("api_path" in req.sws)) {
            req.sws.api_path =
                route_path != "" ? route_path : req.sws.originalUrl;
        }
        ...
        // Push Request/Response record to OpenObserve
        this.openobserveEmitter.processRecord(rrr);
